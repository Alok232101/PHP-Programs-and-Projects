...
if (isset($_SESSION['key'])) {
  if (@$_GET['demail'] && $_SESSION['key'] == 'Ashis@8895') {
    $demail = @$_GET['demail'];
...
if (isset($_SESSION['key'])) {
  if (@$_GET['q'] == 'rmquiz' && $_SESSION['key'] == 'Ashis@8895') {
    $eid = @$_GET['eid'];
...
if (isset($_SESSION['key'])) {
  if (@$_GET['q'] == 'addquiz' && $_SESSION['key'] == 'Ashis@8895') {
    $name = $_POST['name'];
...
if (isset($_SESSION['key'])) {
  if (@$_GET['q'] == 'addqns' && $_SESSION['key'] == 'Ashis@8895') {
    $n = @$_GET['n'];
    $eid = @$_GET['eid'];
    $ch = @$_GET['ch'];
    for ($i = 1; $i <= $n; $i++) {
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2) {
  ...
  } else if (isset($_SESSION['key']) && $_SESSION['key'] != 'Ashis@8895') {
    $q = mysqli_query($con, "SELECT score FROM history WHERE eid='$eid' AND email='$email'") or die('Error156');
    while ($row = mysqli_fetch_array($q)) {
      $s = $row['score'];
    }
  } else {
    // Error handling: 'key' is not set in $_SESSION or it doesn't match 'Ashis@8895'.
